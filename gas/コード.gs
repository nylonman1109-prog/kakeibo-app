 ...
 *      次のユーザーとして実行: 自分
 *      アクセスできるユーザー: 全員
 * 4. 出てきた /exec で終わるURLを、アプリの ⚙️設定 →「レシート読み取りの設定」に貼る
 *    （以前は index.html に直接書いていたが、渡す相手ごとの書き換えを無くすため画面入力に変えた）
...
/* 合言葉。アプリの ⚙️設定 →「レシート読み取りの設定」に入れる合言葉と同じ文字列にする。
   このURLを拾って自動で叩いてくる相手を弾くためのもの。
   以前はアプリのHTMLに直接書いていて、公開HTMLから読めてしまっていたが、
   利用者が自分の端末に入れる形にしたので、そこからは漏れなくなった。
   それでも最後の歯止めは上の DAILY_LIMIT と、Google Cloud 側のクォータ上限です。
